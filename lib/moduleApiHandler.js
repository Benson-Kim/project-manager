...
export function createModuleApiHandlers({
	...
	const listHandler = async (req, { params }) => {
		...
		const { session } = req;
		await authorizeProjectAccess(projectId, session, null, "view");
	...
	const getHandler = async (req, { params }) => {
		console.log("Raw params (before await):", params); // Debug raw params
		const resolvedParams = await params;
		...
		const { session } = req;
		await authorizeProjectAccess(projectId, session, null, "access");
	...
	const createHandler = async (req, { params }) => {
		const resolvedParams = await params;
		console.log("Create resolved params:", resolvedParams);
		const { id: projectId } = resolvedParams;
		if (!projectId) throw new APIError("Project ID is required", 400);
		const { session } = req;
	...
	const updateHandler = async (req, { params }) => {
		const resolvedParams = await params;
		console.log("Update resolved params:", resolvedParams);
		const { id: projectId, itemId } = resolvedParams;
		if (!projectId) throw new APIError("Project ID is required", 400);
		if (!itemId) throw new APIError("Item ID is required", 400);
		const { session } = req;
	...
	const deleteHandler = async (req, { params }) => {
		const resolvedParams = await params;
		console.log("Delete resolved params:", resolvedParams);
		const { id: projectId, itemId } = resolvedParams;
		if (!projectId) throw new APIError("Project ID is required", 400);
		if (!itemId) throw new APIError("Item ID is required", 400);
		const { session } = req;
	...
	return {
		listHandler: apiHandler(listHandler, null),
		getHandler: apiHandler(getHandler, null),
